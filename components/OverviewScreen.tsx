...
export default function OverviewScreen({ navigation, route }: OverviewProps) {
  ...
  React.useEffect(() => {
    ...
    void getUiPreferences().then((preferences) => {
      ...
      if (
        preferences.consistencyViewMode === "summary" ||
        preferences.consistencyViewMode === "tasks"
      ) {
        setViewMode(preferences.consistencyViewMode);
      }
    });
  ...
  const onceTaskHeatmapData = getHeatmapData(
    onceTasks.flatMap((task) => task.completions),
  );
  const onceTaskCompletionCount = Object.values(onceTaskHeatmapData).reduce(
    (sum, count) => sum + count,
    0,
  );
  ...
  const hasCompletionsOnSelectedDate = goal.tasks.some(
    (task) =>
      ...
      task.completions.some(
        (date) => format(date, "yyyy-MM-dd") === selectedDateKey,
      ),
  );
      ...
      >
        <View>
          ...
          <Text style={{ color: theme.textSecondary, marginTop: 4 }}>
            {viewMode === "summary"
              ? "Goal-level consistency summary"
          ...
          {(
            ...
            <Pressable
              ...
              onPress={() => {
                ...
                void updateUiPreferences({ consistencyViewMode: option.key });
              }}
              style={{
                ...
                borderWidth: 1,
                borderColor:
                  viewMode === option.key ? theme.primary : theme.border,
                backgroundColor:
                  viewMode === option.key
                    ? theme.primary + "20"
                    : theme.surface,
        ...
        {viewMode === "summary" ? (
          ...
          >
            <View>
              ...
              >
                Each day shows what percentage of this goal’s recurring tasks
        ...
        {viewMode === "tasks" && recurringTasks.length === 0 ? (
          ...
          >
            ...
            <Text style={{ color: theme.textSecondary, lineHeight: 22 }}>
              Recurring charts apply to daily, weekly, and custom tasks. One-off
              tasks are shown separately below in a combined support-work
              heatmap when they exist.
            </Text>
          </View>
        ) : null}
                  ...
                  >
                    ...
                    {(() => {
                      ...
                      const progressRatio = customProgress
                        ? Math.min(
                            customProgress.completed / customProgress.target,
                            1,
                          )
                        : 0;
                      const progressOffset =
                        streakRingCircumference * (1 - progressRatio);
                        ...
                        <View style={{ alignItems: "center", marginLeft: 12 }}>
                          <View style={{ position: "relative" }}>
                            <Ionicons
                              ...
                              size={28}
                              color={
                                hasStreak ? "#FF6B35" : theme.textSecondary
                              }
        ...
        {viewMode === "tasks" && onceTasks.length > 0 ? (
          ...
          >
            ...
            <Text style={{ color: theme.textSecondary, fontSize: 13 }}>
              Once tasks in this goal: {onceTasks.length} • Completed one-off
              tasks: {onceTaskCompletionCount}
            </Text>
